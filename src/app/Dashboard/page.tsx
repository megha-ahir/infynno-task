...
const Dashboard: FC = () => {
    ...
    const fetchMatches = async (date: Date) => {
        ...
        const response = await api
            .get(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}football/fixtures/date/${isoDate}?api_token=${process.env.NEXT_PUBLIC_API_TOKEN}`
            )
            .catch((err) => {
    ...
    return (
        <div
            className={`${styles.main} w-100 flex flex-col lg:flex-row gap-3 lg:mx-3 lg:mt-3`}
        >
            <div
                className={`${styles.dashboardContiner} mh-full flex flex-col flex-1 gap-3 overflow-scroll`}
            >
                ...
                <div className="bg-white rounded-lg p-3 background-card h-full">
                    <div className="flex flex-col gap-4">
                        <div className="flex gap-2 items-center">
                            <div className="tab flex items-center px-4 py-2 text-theme rounded-xl space-x-2">
                                <Image
                                    ...
                                    className="mr-1"
                                />
                                Live <span className="text-white ps-1"> ({liveCount})</span>
                            </div>

                            <div
                                className={`${styles.searchContainer} tab w-full rounded-xl`}
                            >
                                <Image
                            ...
                            <div className="flex flex-1 text-center mw-full gap-2  overflow-x-scroll scrollbar-none ">
                                {!dateLabels ? (
                                    ...
                                    dateLabels?.map((item: any, index: number) => (
                                        <div
                                            key={index}
                                            className={`${selectedDate?.getDate() ===
                                                    item.currentDateISO.getDate()
                                                    ? "border-theme"
                                                    : ""
                        ...
                        <div className="flex flex-col gap-2 overflow-y-scroll scrollbar-none">
                                ...
                                <div className="bg-black text-white flex items-center justify-between lg:justify-start p-3 py-2">
                                    <div className="flex w-50 lg:w-2/5 items-center gap-2">
                                        <Image
                                            ...
                                            src={require("@/assets/dashboard/header-image.png")}
                                            height={28}
                                            width={28}
                                            alt="Calender"
                                        />
                                            ...
                                            <>
                                                <div className="w-full">
                                                    ...
                                                    >
                                                        ...
                                                        {isLive(item.starting_at as Date) ? (
                                                            <span className="text-theme">Live</span>
                                                        ) : (
                            ...
                            {matches?.message && (
                                <h4 className="tab h-full rounded-lg p-3 px-7 text-alert text-center text-theme">
                                    {" "}
                                    {matches?.message}
                                </h4>
                            )}
